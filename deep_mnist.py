...
mnist = input_data.read_data_sets("D:/Code/tensorflow/MNIST_data/", one_hot=True)
...
y_s = tf.matmul(h_fc1_drop, W_fc2) + b_fc2
y_conv = tf.nn.softmax(y_s)
...
def train(num=20000):
    ...
    #保存参数
    save_path = saver.save(sess, "D:/Code/tensorflow/model.ckpt")
    print ("test accuracy %g" % accuracy.eval(feed_dict={x:mnist.test.images, y_:mnist.test.labels, keep_prob:1.0}))

#恢复模型，预测数据
def predict(num=1):
    saver.restore(sess, "D:/Code/tensorflow/model.ckpt")
    print ("test accuracy %g" % accuracy.eval(feed_dict={x:mnist.test.images, y_:mnist.test.labels, keep_prob:1.0}))
    #恢复图计算
    # saver.restore(sess, "./model.ckpt")
    ix = mnist.test.images[num]
    ix = ix.reshape(1,784)
    iy = mnist.test.labels[num]
    iy = iy.reshape(1,10)
    print(iy)
    result = sess.run(y_s,feed_dict={x:ix,y_:iy,keep_prob:0.5})
    print(result)
    return result
...
if __name__ == "__main__":
    option = sys.argv[1]
    if len(sys.argv) < 3:
        print("num is required.such as 'python .\deep_mnist.py predict 2'")
        exit()
    else:
        num = sys.argv[2]
    num = int(num)
    if option == 'train':
        if num > 200:
            ...
            print("num is smaller than 200,it will run 20000 times")
            train()
    else:
        if num >= 0 and num <= 10000:
            predict(num)
        else:
            print("num is between 0 to 10000,its default value is 0")
            predict()